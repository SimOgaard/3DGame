...
public class FlyCamera : MonoBehaviour
{
    ...
    void Update()
    {
        float sqrDistNow = 3.40282347E+38f;
        Transform nearestChild = CelestialBodies.transform;
        foreach (Transform child in CelestialBodies)
        {
            float sqrDist = (child.position - transform.position).sqrMagnitude;

            // Om du är närmast solen sätt directional ligt intensity till 0.001, och sätt på point light
            if (sqrDist < sqrDistNow && child.GetComponent<CelestialBodyManager>().oceanMaterial)
            {
                sqrDistNow = sqrDist;
                nearestChild = child;
            }
        ...
        oceanEffects.ChangeMaterialToRender(nearestChild);
